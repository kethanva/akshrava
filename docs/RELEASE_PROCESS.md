...
3. Merge the pull request, create an annotated `vX.Y.Z` tag on that exact `main` commit, and push
   the tag. The tag workflow rebuilds the backend wheel and debug APK, then publishes both assets
