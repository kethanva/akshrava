...
def main() -> int:
    ...
    try:
        ...
        # Every other failure path below prints a clean stderr message and returns 1. A bare
        # exception here would instead dump a Python traceback into CI output and mask the actual
        # gate failure (a missing/renamed release file) behind noise.
        print("release version check failed: %s" % exc, file=sys.stderr)
        return 1
