...
# Scans product source only: the library (ios/Akshrava/Akshrava) and the app target
# (ios/AkshravaApp). AkshravaTests is deliberately excluded -- SafetyBoundaryTests.swift and
# friends legitimately quote these terms as literals inside their own assertions, and scanning
# tests would make adding the single-word terms below impossible without constant false positives.
AUDIT_PATHS=(
  "$IOS_DIR/Akshrava"
  "$REPO_ROOT/ios/AkshravaApp"
)
FORBIDDEN_PHRASES=(
  "crossing decision"
  "collision avoidance"
...
# Single-word terms from CLAUDE.md's safety boundary list. Word-bounded (-w) so this does not
# flag incidental substrings; verified clean against current product source before landing.
FORBIDDEN_WORDS=(
  "navigation"
  "collision"
  "safe"
...
VIOLATIONS=0
for TERM in "${FORBIDDEN_PHRASES[@]}"; do
  MATCHES="$(grep -rn -i "$TERM" "${AUDIT_PATHS[@]}" \
    --include="*.swift" \
...
for TERM in "${FORBIDDEN_WORDS[@]}"; do
  MATCHES="$(grep -rn -iw "$TERM" "${AUDIT_PATHS[@]}" \
    --include="*.swift" \
    --exclude-dir=".build" 2>/dev/null | grep -v '^\s*//' | grep -v '///' || true)"
  if [[ -n "$MATCHES" ]]; then
    echo "SAFETY VIOLATION: '$TERM' found:"
