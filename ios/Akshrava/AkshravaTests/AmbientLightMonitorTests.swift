...
final class AmbientLightMonitorTests: XCTestCase {
    ...
    func testClassifyHysteresisBand() {
        XCTAssertEqual(AmbientLightMonitor.classify(brightnessIndex: 5), .dark)
        XCTAssertEqual(AmbientLightMonitor.classify(brightnessIndex: 80), .bright)
        XCTAssertNil(AmbientLightMonitor.classify(brightnessIndex: 30))
    }

    func testFirstConfidentReadingSeedsWithoutAnnounce() {
        let step = AmbientLightMonitor.step(
            state: AmbientLightState(), brightnessIndex: 5, nowMs: 1_000
        )
    ...
    func testEdgeRequiresHoldThenAnnounces() {
        ...
        var step = AmbientLightMonitor.step(state: state, brightnessIndex: 5, nowMs: 1_000)
        XCTAssertNil(step.announce)
        state = step.state
        step = AmbientLightMonitor.step(
            state: state,
            brightnessIndex: 5,
            nowMs: 1_000 + AmbientLightMonitor.holdMs
        )
    ...
    func testBrightnessIndexMapsHighISOTowardDark() {
        let darkish = AmbientLightMonitor.brightnessIndex(iso: 800, exposureSeconds: 0.003)
        let brightish = AmbientLightMonitor.brightnessIndex(iso: 50, exposureSeconds: 0.001)
